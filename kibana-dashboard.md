## Go Over the Kibana dashboard and Common filters.

**We must first review what is the ELK stack.**
ELK stands for 
...
#### Filter by namespace
Now let use filter based on  namespace `kubernetes.namespace_name:"coolstore-1"`
...
#### Filter by namespacelog level
~~~shell
(kubernetes.namespace_name:"coolstore-1" AND level:err)

(kubernetes.namespace_name:"coolstore-1" AND level:info)
~~~

#### Save error filter
(kubernetes.namespace_name:"coolstore-1" AND level:err)
1. click on Save
2. Save search name as `Namespace Errors`
...
![Selected Fields]({% image_path kibana-selected-fields.png %}){:width="900px"}

Experiment with adding and removing selected fields while you are filtering the compenents. 

**Filter by namespace and POD name**  
~~~shell
(kubernetes.namespace_name:"coolstore-1" AND kubernetes.pod_name:vertx)  
~~~

**Filter by namespace and container name**  
~~~shell
(kubernetes.namespace_name:"coolstore-1" AND kubernetes.container_name:vertx)
~~~

**Filter by namespace and container name**  
~~~shell
(kubernetes.namespace_name:"coolstore-1" AND kubernetes.labels.app:catalog)  
~~~

**Filter by namespace and deployment**  
~~~shell
(kubernetes.namespace_name:"coolstore-1" AND  kubernetes.labels.deploymentconfig:gateway)  
~~~

**Chain a query**  
~~~shell
(kubernetes.namespace_name:"coolstore-1" AND kubernetes.labels.app:catalog) OR(kubernetes.namespace_name:"coolstore-1" AND kubernetes.labels.app:gateway) OR(kubernetes.namespace_name:"coolstore-1"  AND kubernetes.labels.app:inventory) OR(kubernetes.namespace_name:"coolstore-1"  AND kubernetes.labels.app:web)
~~~

#### Get status code of Post via vertex using regex
...
~~~shell
(kubernetes.namespace_name:"coolstore-1" AND kubernetes.labels.app:gateway  AND message:*status code*)
~~~
...
Below is an example  steps of configuring a java application to use fluentd.
[Fluentd Java](https://docs.fluentd.org/language-bindings/java)

1. Install the tdagent RPM
2. Modify the config file then restart your td-agent service
    ~~~xml
    <source>
    @type forward
    port 24224
    ...
    ~~~
3. Update your pom.xml with the appropriate dependency information
    ~~~xml
    <dependencies>
    ...
    <dependency>
...
4. Add the following to your application
    ~~~java
...
6. Once your app is running logs will report to td-agent.log 
    ~~~shell
